...
import { defaultValueForBlock } from './editorSchema'
import { BlockFieldRenderer } from './BlockFieldRenderer'

type BlockValue = Record<string, unknown> & { blockType?: string; id?: string }

interface PageEditDrawerProps {
  pageId: string
...
export function PageEditDrawer({ pageId, blockSchemas, open, onClose }: PageEditDrawerProps) {
  const router = useRouter()
  const [title, setTitle] = useState('')
  const [blocks, setBlocks] = useState<BlockValue[] | null>(null)
  ...
  // Load the page's layout at depth=0 so every relation is a plain id that
  // round-trips on save without normalization.
  useEffect(() => {
    if (!open) return
    let cancelled = false
    setLoading(true)
    setError(null)
    fetch(`/api/pages/${pageId}?depth=0`, { credentials: 'include' })
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error(`Load failed (${r.status})`))))
      .then((doc: { title?: string; layout?: BlockValue[] }) => {
        if (cancelled) return
        setTitle(doc.title ?? '')
  ...
  const updateBlock = (index: number, next: BlockValue) => {
  ...
  const addBlock = (schema: EditorBlockSchema) => {
    setBlocks((prev) => {
      const next = [...(prev ?? []), defaultValueForBlock(schema) as BlockValue]
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/pages/${pageId}`, {
        ...
        body: JSON.stringify({ layout: blocks, _status: 'published' }),
      })
      if (!res.ok) {
                    ...
                    <div className="px-4 pb-5 pt-1 border-t border-piano-stone/10">
                      {schema ? (
                        <BlockFieldRenderer
                          ...
                          value={block}
                          onChange={(next) => updateBlock(i, next as BlockValue)}
                        />
                      ) : (
