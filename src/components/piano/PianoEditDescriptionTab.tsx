...
export function PianoEditDescriptionTab({ pianoId, initialValue }: Props) {
  const router = useRouter()
  const [value, setValue] = useState<SerializedEditorState | null>(initialValue ?? null)
  ...
  }, [pianoId, router, value])

  const initialConfig = {
    namespace: 'PianoDescriptionEditor',
    theme: editorTheme,
    onError: (err: Error) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <LexicalComposer initialConfig={initialConfig}>
        <Toolbar />
        <div className="flex-1 relative bg-white border border-piano-stone/15 mx-7 overflow-y-auto">
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                className="outline-none px-5 py-4 min-h-full text-piano-black text-[15px] leading-relaxed"
                aria-label="Piano description editor"
              />
            }
            placeholder={
              <div className="absolute top-4 left-5 text-piano-stone/40 text-[15px] pointer-events-none select-none">
                Write the piano description…
              </div>
            }
        ...
        {/* Footer */}
        <div className="px-7 pt-4 pb-5 space-y-2 shrink-0">
          {error && (
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-piano-black text-piano-cream px-6 py-3 font-display text-xs tracking-[0.3em] uppercase hover:bg-piano-burgundy transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {saving && <Loader2 size={13} className="animate-spin" />}
            {saved ? 'Saved!' : saving ? 'Saving…' : dirty ? 'Save Description' : 'No Changes'}
          </button>
        </div>
      </LexicalComposer>
    </div>
...
function Toolbar() {
  ...
  return (
    <div className="flex items-center gap-1 px-7 py-3 border-y border-piano-stone/15 bg-piano-cream/60 mx-0 mb-3 flex-wrap shrink-0">
