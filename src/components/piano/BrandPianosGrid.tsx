...
export function BrandPianosGrid({ pianos, brandName }: Props) {
  return (
        ...
        {pianos.length > 0 ? (
          ...
          <div className="py-20 text-center border border-piano-linen">
            <p className="text-piano-stone text-lg mb-4">
              No {brandName} pianos in current inventory.
            </p>
            <p className="text-piano-stone/60 text-base max-w-md mx-auto leading-relaxed mb-10">
              ...
              when a {brandName} becomes available.
            </p>
            <Link
              href={`/contact?subject=${encodeURIComponent(`Waiting List: ${brandName}`)}`}
