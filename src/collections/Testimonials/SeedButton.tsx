...
export function SeedButton() {
  ...
  const handleSeed = async () => {
    ...
    try {
      const res = await fetch('/api/seed/testimonials', { method: 'POST' })
  ...
  return (
    <div style={{ padding: '0 0 16px' }}>
      ...
      >
        ...
        {status === 'idle' && '🌱 Seed Testimonials From Word Docs'}
      ...
      {status === 'idle' && (
        <p style={{ margin: '6px 0 0', fontSize: '11px', color: 'var(--theme-elevation-500)' }}>
          Imports 29 real customer testimonials as published entries. Safe to run — skips any that already exist. Add photos to each when ready.
        </p>
      )}
